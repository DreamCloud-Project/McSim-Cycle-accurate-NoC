...
#include "embedded_with_ACK_proc.h"


void EmbeddedWithACKProc::initialize(int x, int y){
	local_x = x;
	local_y = y;
...
//EmbeddedFactors *PE::synthetic_evaluation(){
ProcEvaluationFactors *EmbeddedWithACKProc::evaluation(){
	EmbeddedFactors *synth_factors = new EmbeddedFactors();
...
void EmbeddedWithACKProc::tx_process(){
...
//		if ((EmbeddedParameters::traffic_type == TRAFFIC_TRANSPOSE)
//				&& (local_x == local_y)){
			// do nothing, not transfer packet to itself
//		}
//		else if ((EmbeddedParameters::traffic_type == TRAFFIC_BIT_REVERSAL)
 ...
 * update tx_state
 */
void EmbeddedWithACKProc::tx_state_process(){
 ...
 * update tx_state_reg
 */
void EmbeddedWithACKProc::tx_state_reg_process(){
	if (reset.read()){
 ...
 * tx_mux_sel process
 */
void EmbeddedWithACKProc::tx_mux_sel_process(){
 ...
 * flit_out
 */
void EmbeddedWithACKProc::flit_out_process(){
/*
	if (out_vc_remain[0].read()>=1){
 ...
 */
void EmbeddedWithACKProc::has_sent_tail_flit_process(){
	if (reset.read()){
		has_sent_tail_flit.write(0);
	}
	else {
 ...
 */
void EmbeddedWithACKProc::has_sent_head_flit_process(){
	if (reset.read()){
 ...
 */
void EmbeddedWithACKProc::rx_process(){
	if (reset.read()){
 ...
 */
void EmbeddedWithACKProc::received_ACK_process() {
	if (received_ACK_reg.read()) {
		if (has_sent_head_flit.read())
 ...
 */
void EmbeddedWithACKProc::received_ACK_reg_process(){
 ...
 */
void EmbeddedWithACKProc::out_vc_remain_process(){
	for (int vo=0; vo<RouterParameter::n_VCs; vo++){
 ...
 */
void EmbeddedWithACKProc::out_vc_remain_reg_process(){
	if (reset.read()){
		for (int vo=0; vo<RouterParameter::n_VCs; vo++){
 ...
 */
void EmbeddedWithACKProc::count_plus_process(){
	for (int vo=0; vo<RouterParameter::n_VCs; vo++){
 ...
 * count_minus
 */
void EmbeddedWithACKProc::count_minus_process(){
	if (reset.read()){
		for (int vo=0; vo<RouterParameter::n_VCs; vo++){
			count_minus[vo].write(0);
		}
